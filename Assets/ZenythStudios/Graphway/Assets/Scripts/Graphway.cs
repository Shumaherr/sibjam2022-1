...
public class Graphway : MonoBehaviour
{
    ...
    [Tooltip("Color of NODES in editor.")] public Color nodeColor = new Color32(236, 13, 69, 255);

    [Tooltip("Color of SUBNODES in editor.")]
    public Color subnodeColor = new Color32(40, 12, 54, 255);

    [Tooltip("Size of NODES in editor.")] public float nodeSize = 10;
    ...
    [Tooltip(
        "Number of nodes that can be checked in a single frame at runtime.  A higher limit will find paths faster, but may impact performance.")]
    ...
    void Awake()
    {
        ...
        else
        {
            ...
            foreach (Transform connection in connectionsParent.transform)
            {
                ...
                {
                    nodes[connectionData.nodeIDA].AddConnection(connectionData.nodeIDB, connectionData.disabled,
                        connectionData.speedWeight, subnodesAB);
                }
                ...
                {
                    nodes[connectionData.nodeIDB].AddConnection(connectionData.nodeIDA, connectionData.disabled,
                        connectionData.speedWeight, subnodesBA);
                }
            }
    ...
    public static void FindPath(Vector3 origin, Vector3 targetPosition, Action<GwWaypoint[]> callback,
                    ...
                    foreach (KeyValuePair<int, GwConnection> connection in nodes[currentNodeID].connections)
                    {
                        ...
                        if (connection.Value.disabled == false && closedNodes.Contains(connectedNodeID) == false)
                        {
                            ...
                            float gCost = nodes[currentNodeID].gCost +
                                          (CalculatePathLength(currentNodeID, connectedNodeID) /
                                           connection.Value.speedWeight);
            ...
            foreach (KeyValuePair<int, GwNode> node in nodes)
            {
                ...
                foreach (KeyValuePair<int, GwConnection> connection in node.Value.connections)
                {
                    if (!connection.Value.disabled)
                    {
                            ...
                            for (int i = 0; i < connection.Value.subnodes.Length; i++)
                            {
                                ...
                                if (!nodes[connection.Value.connectedNodeID].connections.ContainsKey(node.Value.nodeID))
                                {
                                    DrawArrow(RaiseHeight(lastPosition),
                                        RaiseHeight(subnodePosition - lastPosition) / 2,
                                        (i == 0 ? nodeColor : subnodeColor));
                                }
                        ...
                        if (!nodes[connection.Value.connectedNodeID].connections.ContainsKey(node.Value.nodeID))
                        {
                            DrawArrow(RaiseHeight(lastPosition), RaiseHeight(nodeBPosition - lastPosition) / 2,
                                nodeColor);
                        }
                    }
                }
